...
const TML_COLLECTION_TYPES: { name: string; doc: string }[] = [
    ...
    { name: 'Vec', doc: 'Dynamic array (alias for List): Vec[T]' },
    { name: 'HashMap', doc: 'Hash map: HashMap[K, V]' },
    { name: 'HashSet', doc: 'Hash set: HashSet[T]' },
    { name: 'BTreeMap', doc: 'Sorted map (binary search): BTreeMap' },
    { name: 'BTreeSet', doc: 'Sorted set (binary search): BTreeSet' },
    { name: 'Deque', doc: 'Double-ended queue (ring buffer): Deque[T]' },
    { name: 'Buffer', doc: 'Byte buffer for binary data' },
];
...
const TML_WRAPPER_TYPES: { name: string; doc: string }[] = [
    ...
    { name: 'Heap', doc: 'Heap-allocated owned value (like Box)' },
    { name: 'Shared', doc: 'Reference-counted pointer (like Rc)' },
    { name: 'Sync', doc: 'Thread-safe atomic reference-counted pointer (like Arc)' },
    { name: 'Text', doc: 'Dynamic, growable string with Small String Optimization (SSO)' },
];
...
const TML_BUILTINS: { name: string; doc: string; signature: string }[] = [
    ...
    { name: 'assert_true', doc: 'Assert value is true', signature: 'func assert_true(cond: Bool)' },
    { name: 'assert_false', doc: 'Assert value is false', signature: 'func assert_false(cond: Bool)' },
    { name: 'assert_lt', doc: 'Assert left < right', signature: 'func assert_lt[T](left: T, right: T)' },
    { name: 'assert_gt', doc: 'Assert left > right', signature: 'func assert_gt[T](left: T, right: T)' },
    { name: 'assert_lte', doc: 'Assert left <= right', signature: 'func assert_lte[T](left: T, right: T)' },
    { name: 'assert_gte', doc: 'Assert left >= right', signature: 'func assert_gte[T](left: T, right: T)' },
    { name: 'assert_in_range', doc: 'Assert min <= val <= max', signature: 'func assert_in_range[T](val: T, min: T, max: T)' },
    { name: 'assert_str_len', doc: 'Assert string length equals expected', signature: 'func assert_str_len(s: Str, len: I64)' },
    { name: 'assert_str_empty', doc: 'Assert string is empty', signature: 'func assert_str_empty(s: Str)' },
    { name: 'assert_str_not_empty', doc: 'Assert string is not empty', signature: 'func assert_str_not_empty(s: Str)' },
...
const TML_MODULES: { name: string; doc: string; members: string[] }[] = [
    { name: 'std', doc: 'Standard library root module', members: ['io', 'fs', 'collections', 'time', 'datetime', 'math', 'net', 'env', 'fmt', 'iter', 'sync', 'thread', 'json', 'regex', 'crypto', 'compress', 'random', 'search', 'hash', 'url', 'uuid', 'semver', 'glob', 'os', 'text', 'process', 'path', 'log'] },
    { name: 'std::io', doc: 'Input/output operations', members: ['stdin', 'stdout', 'stderr', 'Read', 'Write', 'BufReader', 'BufWriter'] },
    { name: 'std::fs', doc: 'File system operations', members: ['File', 'read', 'write', 'exists', 'create_dir', 'remove', 'Path', 'Dir'] },
    { name: 'std::collections', doc: 'Collection types', members: ['List', 'Vec', 'HashMap', 'HashSet', 'BTreeMap', 'BTreeSet', 'Deque', 'Buffer'] },
    { name: 'std::time', doc: 'Time and duration utilities', members: ['Instant', 'Duration'] },
    { name: 'std::datetime', doc: 'Date and time handling', members: ['SystemTime', 'DateTime'] },
    { name: 'std::math', doc: 'Mathematical functions', members: ['abs', 'min', 'max', 'sqrt', 'pow', 'sin', 'cos', 'tan', 'PI', 'E', 'floor', 'ceil', 'round', 'log', 'log2', 'log10', 'exp'] },
    ...
    { name: 'std::sync', doc: 'Synchronization primitives', members: ['Mutex', 'RwLock', 'Shared', 'Sync', 'Barrier', 'Condvar'] },
    { name: 'std::thread', doc: 'Threading utilities', members: ['spawn', 'current', 'sleep', 'yield_now', 'JoinHandle'] },
    { name: 'std::json', doc: 'JSON parsing and serialization', members: ['parse', 'stringify', 'JsonValue', 'JsonArray', 'JsonObject'] },
    { name: 'std::regex', doc: 'Regular expressions (Thompson NFA)', members: ['Regex', 'Captures', 'Match'] },
    { name: 'std::crypto', doc: 'Cryptographic primitives', members: ['hash', 'hmac', 'cipher', 'sign', 'kdf', 'random'] },
    { name: 'std::crypto::hash', doc: 'Cryptographic hash functions', members: ['sha256', 'sha512', 'sha384', 'md5', 'sha1', 'blake2b', 'Digest'] },
    { name: 'std::compress', doc: 'Compression algorithms', members: ['deflate', 'inflate', 'gzip', 'gunzip', 'brotli', 'zstd', 'lz4'] },
    { name: 'std::random', doc: 'Random number generation', members: ['Xoshiro256', 'ThreadRng', 'seed', 'next_i64', 'next_f64'] },
    { name: 'std::search', doc: 'Search algorithms', members: ['bm25', 'hnsw', 'cosine_distance', 'BM25', 'HNSW'] },
    { name: 'std::hash', doc: 'Non-cryptographic hashing', members: ['fnv1a', 'siphash'] },
    { name: 'std::url', doc: 'URL parsing and manipulation', members: ['Url', 'parse', 'host', 'port', 'path', 'query'] },
    { name: 'std::uuid', doc: 'UUID generation', members: ['Uuid', 'v4', 'v7'] },
    { name: 'std::semver', doc: 'Semantic versioning', members: ['Version', 'parse'] },
    { name: 'std::glob', doc: 'File glob pattern matching', members: ['glob', 'Pattern'] },
    { name: 'std::os', doc: 'OS-level operations', members: ['platform', 'arch'] },
    { name: 'std::text', doc: 'Extended text operations (Text type with SSO)', members: ['Text'] },
    { name: 'std::process', doc: 'Process management', members: ['Command', 'exit', 'abort'] },
    { name: 'std::path', doc: 'Path manipulation', members: ['Path', 'PathBuf', 'join', 'parent', 'extension'] },
    { name: 'std::log', doc: 'Logging framework', members: ['info', 'warn', 'error', 'debug', 'trace'] },
    { name: 'core', doc: 'Core language primitives', members: ['mem', 'ptr', 'slice', 'ops', 'cmp', 'convert', 'default', 'encoding', 'fmt', 'iter'] },
    { name: 'core::mem', doc: 'Memory manipulation', members: ['size_of', 'align_of', 'drop', 'forget', 'swap', 'replace'] },
    { name: 'core::ptr', doc: 'Raw pointer operations', members: ['Ptr', 'null', 'null_mut', 'read', 'write', 'copy'] },
    { name: 'core::encoding', doc: 'Binary encoding (big/little endian, base64, hex, utf8)', members: ['big_endian', 'little_endian', 'base64', 'hex', 'utf8'] },
    { name: 'test', doc: 'Testing framework', members: ['assert', 'assert_eq', 'assert_ne', 'assert_true', 'assert_false', 'assert_lt', 'assert_gt'] },
];
