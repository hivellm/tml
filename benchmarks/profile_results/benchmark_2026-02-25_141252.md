...
| Int to String | 9 | 44 | C++ 5x |
| String Copy | 3 | - | C++ only |
| String Repeat (50 chars) | 27 | - | C++ only |
| Sprintf Formatting | 91 | - | C++ only |
| Log Building | 24 | - | C++ only |
| Concat Loop (Text - O(n)) | - | 1 | TML only |
| Concat Loop (Str - O(n^2)) | - | 4119 | TML only |
| Log Building (Text - O(n)) | - | 50 | TML only |
| Log Building (Str - O(n^2)) | - | 4183 | TML only |
...
| Vec Pop | 1 | - | C++ only |
| Vec Set | 0 | - | C++ only |
| HashMap Insert | 135 | - | C++ only |
| HashMap Insert (reserved) | 98 | - | C++ only |
| HashMap Lookup | 5 | - | C++ only |
| HashMap Contains | 4 | - | C++ only |
| HashMap Remove | 83 | - | C++ only |
| HashMap String Key | 150 | - | C++ only |
...
| Operation | C++ (ns/op) | TML (ns/op) | Verdict |
|---|---:|---:|---|
| HashMap Insert | 297 | 35 | **TML 8.5x faster** |
| HashMap Insert (reserved) | 235 | 36 | **TML 6.5x faster** |
| HashMap Lookup | 5 | 1 | **TML 5.0x faster** |
| HashMap Contains | 5 | 1 | **TML 5.0x faster** |
| HashMap Remove | 373 | 15 | **TML 24.9x faster** |
| HashMap String Key | 500 | - | C++ only |

## List

| Operation | C++ (ns/op) | TML (ns/op) | Verdict |
|---|---:|---:|---|
| List Push (grow) | 10 | 3 | **TML 3.3x faster** |
| List Push (reserved) | 2 | 1 | **TML 2.0x faster** |
| List Random Access | 0 | 0 | ~tied |
| List Iteration | 0 | 0 | ~tied |
| List Pop | 3 | 0 | **TML 3.0x faster** |
| List Set | 0 | 0 | ~tied |
...
| Operation | C++ (ns/op) | TML (ns/op) | Verdict |
|---|---:|---:|---|
| stringstream Append | 175 | - | C++ only |
| string Reserve+Append | 15 | - | C++ only |
| string Naive Append | 4 | - | C++ only |
| Build JSON | 342 | - | C++ only |
| Build HTML | 20 | - | C++ only |
| Build CSV | 45 | - | C++ only |
| Small Appends (1 char) | 1 | - | C++ only |
| Number Formatting | 527 | 86 | **TML 6.1x faster** |
| Log Messages | 50 | 37 | **TML 1.4x faster** |
| Path Building | 24 | 9 | **TML 2.7x faster** |
| Text Append (O(1) amortized) | - | 45 | TML only |
| Str Naive Append (O(n^2)) | - | 3280 | TML only |
| Build JSON (10K items) | - | 93 | TML only |
| Build HTML (10K items) | - | 5 | TML only |
...
| Operation | C++ (ns/op) | TML (ns/op) | Verdict |
|---|---:|---:|---|
| Base64 Encode (13 bytes) | 38 | 44 | ~tied |
| Base64 Encode (95 bytes) | 97 | 75 | **TML 1.3x faster** |
| Base64 Decode (20 chars) | 61 | 38 | **TML 1.6x faster** |
| Hex Encode (13 bytes) | 35 | 41 | ~tied |
| Hex Decode (26 chars) | 61 | 50 | ~tied |
| Base32 Encode (13 bytes) | 40 | 44 | ~tied |

## Crypto

| Operation | C++ (ns/op) | TML (ns/op) | Verdict |
|---|---:|---:|---|
| SHA256 (13 bytes) | 322 | 345 | ~tied |
| SHA256 (95 bytes) | 339 | 356 | ~tied |
| SHA256 Streaming (3 updates) | 325 | 428 | C++ 1.3x |
| SHA512 (13 bytes) | 423 | 468 | ~tied |
| MD5 (13 bytes) | 331 | 357 | ~tied |
| SHA256 + to_hex (13 bytes) | 379 | 396 | ~tied |
...
|---|---|---|
| Parse Tiny (27 bytes) | 286 | 3,484,976 |
| Parse Small (200 bytes) | 1127 | 886,906 |
| Parse Medium (500 bytes) | 3099 | 322,584 |
| Parse Standard (non-SIMD) | 2083 | 480,007 |
| Field Access | 1101 | 908,205 |
| Array Iteration | 1204 | 830,527 |
| Nested Object Access | 1394 | 717,075 |
| Parse + Validate | 1101 | 907,623 |
| Object Traversal | 1088 | 918,785 |
