...
<!-- MIGRATED-FROM-CLAUDE-MD on 2026-04-07T22:34:27.697Z by rulebook v5.3.0 -->
...
- **Creating a task → `mcp__rulebook__rulebook_task_create`** (NOT an agent, NOT mkdir)
...
- Showing a task → `mcp__rulebook__rulebook_task_show` (NOT an agent)
- Updating a task → `mcp__rulebook__rulebook_task_update` (NOT an agent)

**NEVER use `mkdir` to create task directories manually. ALWAYS use `mcp__rulebook__rulebook_task_create` which creates the correct structure (proposal.md, tasks.md, specs/, .metadata.json) automatically.**
