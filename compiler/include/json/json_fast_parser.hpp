...
inline uint32_t parse_hex4_swar(const char* p) {
    ...
    uint8_t v0 = kHexValues[static_cast<unsigned char>(p[0])];
    uint8_t v1 = kHexValues[static_cast<unsigned char>(p[1])];
    uint8_t v2 = kHexValues[static_cast<unsigned char>(p[2])];
    uint8_t v3 = kHexValues[static_cast<unsigned char>(p[3])];
