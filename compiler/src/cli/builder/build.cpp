...
static int run_build_impl(const std::string& path, const BuildOptions& options) {
    ...
    if (!has_tml_imports_needing_codegen && !has_local_generics && !emit_ir_only) {
        ...
        mir_opts.coverage_enabled = CompilerOptions::coverage;
    ...
    if (output_type == BuildOutputType::RlibLib) {
        ...
        if (registry) {
            ...
            for (const auto& [mod_path, _] : registry->get_all_modules()) {
                ...
                // Skip if already handled by the source file's own build.tml
                ...
                auto pkg_bsr =
                    detect_and_run_build_script((pkg_dir / "src" / "mod.tml").string(), verbose);
                if (pkg_bsr.success) {
                    ...
                    for (const auto& artifact : pkg_bsr.copy_artifacts) {
                        ...
                        if (fs::exists(src)) {
                            std::error_code ec;
                            fs::copy_file(src, build_dir / artifact.filename(),
