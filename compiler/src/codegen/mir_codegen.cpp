...
auto MirCodegen::generate(const mir::Module& module) -> std::string {
    ...
    // First pass: collect string constants, enum types, struct types, and generic enum defs
    ...
    for (const auto& func : module.functions) {
        ...
        for (const auto& block : func.blocks) {
            for (const auto& inst : block.instructions) {
                ...
                // Collect struct types from StructInitInst (for imported structs)
                if (auto* struct_inst = std::get_if<mir::StructInitInst>(&inst.inst)) {
                    if (used_struct_types_.find(struct_inst->struct_name) ==
                        ...
                        for (const auto& ft : struct_inst->field_types) {
                            field_types.push_back(ft ? mir_type_to_llvm(ft) : "i64");
                        }
                        used_struct_types_[struct_inst->struct_name] = std::move(field_types);
                    }
                }
...
void MirCodegen::emit_preamble() {
    ...
    if (is_macho) {
        emitln("@__tml_ident = weak constant [" + std::to_string(ident.size() + 1) +
...
void MirCodegen::emit_type_defs(const mir::Module& module) {
    ...
    // Emit definitions for struct types used in StructInitInst but not in module.structs
    // (imported structs like Range that are constructed but not locally defined)
    for (const auto& [struct_name, field_types] : used_struct_types_) {
        ...
        if (!emitted_types_.count(type_name)) {
            ...
            emitln(" }");
            emitted_types_.insert(type_name);
            struct_field_types_[struct_name] = field_types;
        }
    }
