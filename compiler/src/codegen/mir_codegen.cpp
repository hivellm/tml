    ...
    std::visit(
        [this, &result_reg, &result_type, &inst](const auto& i) {
            ...
            if constexpr (std::is_same_v<T, mir::BinaryInst>) {
                ...
                // TML generates empty drop functions for the RAII pattern, but calling them
                // still has overhead from passing struct values by value.
                // Since all current drop functions are empty (just "ret void"), we skip them.
                ...
                for (size_t j = 0; j < i.args.size(); ++j) {
                    ...
                    // Get the actual type of the value from value_types_ (authoritative)
                    // Then fall back to MIR type if not in value_types_
                    std::string actual_type;
                    ...
                    if (actual_type.find("%struct.") == 0) {
                        ...
                        // MIR may have wrong/default type - use actual type from value_types_
                        // This is critical for values returned from runtime functions like
                        // i64_to_string
                        arg_type = actual_type;
                    }
                ...
                // Check if this is a primitive TML type (calls runtime function expecting value)
                ...
                if (is_primitive_tml) {
                    // Primitive TML types call runtime functions (e.g., i64_to_string)
                    // Runtime functions expect the value directly, not a pointer
                    ...
                    // Non-primitive, non-struct types may need spilling to ptr
                    // (e.g., user-defined classes expecting pointer receivers)
                ...
                // Check value_types_ for actual runtime type (critical for intrinsic results)
                // Value_types_ is authoritative for types we've already computed
                ...
                // Use actual operand type if it differs from MIR type
                // This is crucial for intrinsic results like sqrt which return double
                // but MIR may incorrectly default to i32
                ...
                if (tgt_type == "ptr" && operand_actual_type.find("%struct.") == 0) {
                    ...
                    // Override cast kind based on actual types when MIR has incorrect kind
                    // This handles cases where intrinsic return types aren't properly tracked
                    ...
                    if (src_is_float && tgt_is_int) {
                        // Float to int: use fptosi (assuming signed target)
                        cast_name = "fptosi";
                    } else if (src_is_int && tgt_is_float) {
                        // Int to float: use sitofp (assuming signed source)
                        cast_name = "sitofp";
                    } else if (src_is_float && tgt_is_float) {
                        // Float to float: use fpext or fptrunc based on size
                ...
                // Initialize struct using alloca + getelementptr + store pattern.
                // This enables LLVM's SROA to break aggregates into scalars.
                ...
                // Initialize tuple using alloca + getelementptr + store pattern.
                // This enables LLVM's SROA to break aggregates into scalars.
                ...
                // Initialize each element using getelementptr + store
                for (size_t j = 0; j < i.elements.size(); ++j) {
                    ...
                    emitln("    " + elem_ptr_reg + " = getelementptr inbounds " + tuple_type +
                           ", ptr " + alloc_reg + ", i32 0, i32 " + std::to_string(j));
                ...
                // Load and return tuple by value
