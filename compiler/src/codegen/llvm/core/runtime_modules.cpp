...
//!
//! This file emits LLVM IR for imported module functions.
//!
//! ## Emitted Sections
//!
//! | Method                          | Emits                         |
...
//!
//! Related files:
//! - `runtime_modules_strings.cpp`  — `emit_string_constants`
//! - `runtime_modules_library.cpp`  — `emit_referenced_library_definitions` +
...
// Static helpers for const value extraction (used by emit_module_pure_tml_functions)
...
static std::string try_extract_const_value(const parser::Expr* expr, const parser::TypePtr& type,
    ...
    if (expr->is<parser::TupleExpr>()) {
        ...
        std::string llvm_type = "{ ", llvm_value = "{ ";
        for (size_t i = 0; i < elem_values.size(); ++i) {
            if (i > 0) {
                llvm_type += ", ";
                llvm_value += ", ";
...
// emit_string_constants is in runtime_modules_strings.cpp
// emit_referenced_library_definitions is in runtime_modules_library.cpp
