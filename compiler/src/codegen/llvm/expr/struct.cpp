...
//! Field access is in struct_field.cpp.
//!
//! ## Struct Construction
//!
...
auto LLVMIRGen::gen_struct_expr_ptr(const parser::StructExpr& s) -> std::string {
    ...
    if (base_name == "Self" && !current_impl_type_.empty()) {
        ...
        for (size_t i = 0; i < s.fields.size(); ++i) {
            ...
            if (!field_type.empty() && field_type.starts_with("%struct.")) {
                ...
                // Nullable Maybe field: set expected to "ptr" so Nothing/Just use null repr
    ...
    for (size_t i = 0; i < s.fields.size(); ++i) {
        ...
        if (s.fields[i].second->is<parser::StructExpr>()) {
            ...
            // Set expected_enum_type_ if field is a struct type
            std::string saved_expected_enum_type = expected_enum_type_;
            ...
            // may have been used to generate the literal with the correct type already
                ...
                // Handle ptr -> { ptr, ptr } promotion (thin func pointer -> fat pointer with null
                // env)
                else if (actual_llvm_type == "ptr" && target_field_type == "{ ptr, ptr }") {
                    std::string fat = fresh_reg();
                    emit_line("  " + fat +
