...
auto MirTextReader::read_module() -> Module {
    ...
    // TODO: Implement full text parser
    // For now, just read the module name from the first comment
    while (next_line()) {
        skip_whitespace();
        if (expect("; MIR Module:")) {
            skip_whitespace();
    ...
    set_error("Text MIR parsing not fully implemented");
    return module;
}
