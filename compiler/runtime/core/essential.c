...
// ============================================================================
// CPU Profile — Always-on ring buffer + Chrome Trace JSON export
// ============================================================================
//
// Records zone begin/end events with nanosecond timestamps.
// Export to Chrome Trace format (chrome://tracing) via tml_cpuprofile_export().
// Works independently of Tracy — no external viewer needed.

#ifdef _WIN32
#include <windows.h>
static int64_t profile_freq = 0;
static inline int64_t profile_now(void) {
    LARGE_INTEGER li;
    if (!profile_freq) {
        QueryPerformanceFrequency(&li);
        profile_freq = li.QuadPart;
    }
    QueryPerformanceCounter(&li);
    return (int64_t)((double)li.QuadPart / profile_freq * 1000000000.0); // ns
}
#else
#include <time.h>
static inline int64_t profile_now(void) {
...
// Ring buffer of profile events
#define PROFILE_MAX_EVENTS 65536
#define PROFILE_EVENT_BEGIN 0
#define PROFILE_EVENT_END 1
#define PROFILE_EVENT_MSG 2

typedef struct {
    int64_t timestamp_ns;
    const char* name; // zone name (static string, not freed)
    int32_t type;     // BEGIN, END, MSG
    int32_t thread_id;
    int64_t zone_id; // pairs begin/end
} ProfileEvent;

static ProfileEvent g_profile_events[PROFILE_MAX_EVENTS];
static volatile int64_t g_profile_count = 0;
static volatile int32_t g_profile_enabled = 0;
static int64_t g_profile_next_zone_id = 1;

#ifdef _WIN32
static int32_t get_thread_id(void) {
...
void tml_cpuprofile_start(void) {
    g_profile_count = 0;
    g_profile_next_zone_id = 1;
    g_profile_enabled = 1;
}

void tml_cpuprofile_stop(void) {
    g_profile_enabled = 0;
}

// Record a zone begin event. Returns zone_id.
int64_t tml_cpuprofile_begin(const char* name) {
    if (!g_profile_enabled)
        return 0;
    int64_t idx = g_profile_count;
    if (idx >= PROFILE_MAX_EVENTS)
        return 0; // buffer full
    int64_t zid = g_profile_next_zone_id++;
    g_profile_events[idx].timestamp_ns = profile_now();
    g_profile_events[idx].name = name;
    g_profile_events[idx].type = PROFILE_EVENT_BEGIN;
    g_profile_events[idx].thread_id = get_thread_id();
    g_profile_events[idx].zone_id = zid;
    g_profile_count = idx + 1;
    return zid;
}

// Record a zone end event.
void tml_cpuprofile_end(int64_t zone_id) {
    if (!g_profile_enabled)
        return;
    int64_t idx = g_profile_count;
    if (idx >= PROFILE_MAX_EVENTS)
        return;
    g_profile_events[idx].timestamp_ns = profile_now();
    g_profile_events[idx].name = "";
    g_profile_events[idx].type = PROFILE_EVENT_END;
    g_profile_events[idx].thread_id = get_thread_id();
    g_profile_events[idx].zone_id = zone_id;
    g_profile_count = idx + 1;
}

// Record a message event.
void tml_cpuprofile_message(const char* text) {
    if (!g_profile_enabled)
        return;
    int64_t idx = g_profile_count;
    if (idx >= PROFILE_MAX_EVENTS)
        return;
    g_profile_events[idx].timestamp_ns = profile_now();
    g_profile_events[idx].name = text;
    g_profile_events[idx].type = PROFILE_EVENT_MSG;
    g_profile_events[idx].thread_id = get_thread_id();
    g_profile_events[idx].zone_id = 0;
    g_profile_count = idx + 1;
}

// Export to Chrome Trace JSON format (chrome://tracing)
int64_t tml_cpuprofile_export(const char* filename) {
    FILE* f = fopen(filename, "w");
    if (!f)
        return -1;

    fprintf(f, "{\"traceEvents\":[\n");
    int64_t count = g_profile_count;
    int first = 1;

    for (int64_t i = 0; i < count; i++) {
        ProfileEvent* ev = &g_profile_events[i];
        if (!first)
            fprintf(f, ",\n");
        first = 0;

        if (ev->type == PROFILE_EVENT_BEGIN) {
            fprintf(
                f,
                "{\"name\":\"%s\",\"cat\":\"tml\",\"ph\":\"B\",\"ts\":%.3f,\"pid\":1,\"tid\":%d}",
                ev->name, ev->timestamp_ns / 1000.0, ev->thread_id);
        } else if (ev->type == PROFILE_EVENT_END) {
            // Find matching begin to get name
            const char* name = "?";
            for (int64_t j = i - 1; j >= 0; j--) {
                if (g_profile_events[j].zone_id == ev->zone_id &&
                    g_profile_events[j].type == PROFILE_EVENT_BEGIN) {
                    name = g_profile_events[j].name;
                    break;
                }
            }
            fprintf(
                f,
                "{\"name\":\"%s\",\"cat\":\"tml\",\"ph\":\"E\",\"ts\":%.3f,\"pid\":1,\"tid\":%d}",
                name, ev->timestamp_ns / 1000.0, ev->thread_id);
        } else if (ev->type == PROFILE_EVENT_MSG) {
            fprintf(f,
                    "{\"name\":\"%s\",\"cat\":\"tml\",\"ph\":\"i\",\"ts\":%.3f,\"pid\":1,\"tid\":%"
                    "d,\"s\":\"g\"}",
                    ev->name, ev->timestamp_ns / 1000.0, ev->thread_id);
        }
    }
...
// Get event count
int64_t tml_cpuprofile_count(void) {
