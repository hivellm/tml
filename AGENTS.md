...
Generated at: 2026-01-08T17:12:42.344Z

## Sandbox Directory (`.sandbox/`)

The `.sandbox/` directory is the AI agent's scratch space. Agents (Claude Code, etc.) **MUST** use this directory for all temporary file generation:
- Temp source files, IR dumps, Rust samples, experiment scripts
- Debug output, investigation artifacts
- Any throwaway files created during development

**`.sandbox/` is gitignored.** Agents have full autonomy inside it — no permission needed to create, modify, or delete files there. **NEVER create temp files in the project root.**
...
10. **⛔ MANDATORY: Minimize C and C++ code** — TML is migrating toward self-hosting. Do NOT add new C/C++ code unless absolutely necessary. Follow the three-tier rule:
    - ✅ **Pure TML** (preferred): Implement algorithms in `.tml` files using memory intrinsics (`ptr_read`, `ptr_write`, `mem_alloc`, etc.)
    - ✅ **`@extern("c")` FFI** (acceptable): Call existing external libraries (LLVM, OpenSSL, zlib, libc, OS APIs) via FFI bindings declared in TML
    - ❌ **New C/C++ code** (last resort only): Only for OS-level I/O, panic handlers, or test harness DLL entry points
    - ❌ NEVER add new `.c` files to `compiler/runtime/` for algorithms TML can express
    - ❌ NEVER add `lowlevel` blocks calling C functions for pure logic (strings, collections, formatting, math)
    - ❌ NEVER add C++ compiler code for features that could be TML library code
    - See `CLAUDE.md` and [docs/ROADMAP.md](docs/ROADMAP.md) for full details
...
### TML Test Development - MANDATORY Incremental Workflow

**This workflow is NON-NEGOTIABLE when writing TML tests:**
...
4. **Use individual test execution** - NEVER run full test suite when developing new tests
...
- Coverage is the primary metric for analyzing application progress
...
## MANDATORY: Rust-as-Reference IR Methodology for Codegen

**When fixing codegen bugs or optimizing the TML compiler's LLVM IR output, agents MUST use Rust as the reference implementation.**

This is a HARD REQUIREMENT aligned with task `optimize-codegen-like-rust`. The TML compiler aims to produce IR of the same quality as `rustc`.

### Workflow

For every codegen issue (bug fix or optimization), the agent MUST:

1. **Write equivalent code in both languages** in `.sandbox/`:
   - `.sandbox/temp_<feature>.rs` — Rust version
   - `.sandbox/temp_<feature>.tml` — TML version (equivalent semantics)

2. **Generate LLVM IR from both compilers** (debug and release)

3. **Compare function-by-function** to identify:
   - Instruction count differences
   - Type layout differences (struct/enum sizes)
   - Unnecessary allocas, loads, stores
   - Missing safety features (overflow checks, bounds checks)
   - Redundant call overhead

4. **Fix the TML codegen** to match or exceed Rust's IR quality

5. **Run the test suite** to verify no regressions

### When to Apply

- ANY bug in `compiler/src/codegen/`
- ANY new codegen feature (new expressions, types, patterns)
- ANY IR optimization work
- ANY "wrong code" or "miscompilation" investigation
...
| Issue | TML Status | Rust Reference | Priority |
|-------|-----------|---------------|----------|
| `Maybe[I32]` = 16 bytes | `{ i32, [1 x i64] }` | `{ i32, i32 }` = 8 bytes | HIGH |
| Struct init uses alloca+store+load | 10 instructions | `insertvalue` = 3 instructions | HIGH |
| 500+ unused runtime declarations | Always emitted | On-demand only | MEDIUM |
| No overflow checking | `add nsw` (UB) | `@llvm.sadd.with.overflow` + panic | MEDIUM |
| No exception handling | Bare `call` | `invoke` + `cleanuppad` (SEH) | LOW |

**Full task details**: `rulebook/tasks/optimize-codegen-like-rust/`

**This task is incremental and deferred.** Whenever you discover a codegen inefficiency, bug, or interesting pattern during ANY work (iterator fixes, closure fixes, generic instantiation, etc.), you MUST update the task's `tasks.md` with the new finding — add new checklist items, notes, or new phases. The task is a running log of everything that needs optimization. Dedicated execution happens later when the compiler is stable.

**VIOLATION OF THIS METHODOLOGY IS UNACCEPTABLE when working on codegen tasks.**
