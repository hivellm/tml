...
Generated at: 2026-03-24T09:57:19.599Z
...
- **ADR-001: In-process LLVM vs Subprocess Compilation** (proposed) → [details](/.rulebook/decisions/001-in-process-llvm-vs-subprocess-compilation.md)
- **ADR-002: Query-based Incremental Compilation (Red-Green Model)** (proposed) → [details](/.rulebook/decisions/002-query-based-incremental-compilation-red-green-model.md)
- **ADR-003: Pure C ABI for Plugin Interface** (proposed) → [details](/.rulebook/decisions/003-pure-c-abi-for-plugin-interface.md)
- **ADR-004: NDJSON Subprocess Test Protocol (Go Model)** (proposed) → [details](/.rulebook/decisions/004-ndjson-subprocess-test-protocol-go-model.md)
- **ADR-005: Dual Codegen Paths: Legacy AST→IR + MIR→IR** (proposed) → [details](/.rulebook/decisions/005-dual-codegen-paths-legacy-ast-ir-mir-ir.md)
- **ADR-006: TML-First Runtime: Migrating Away from C/C++** (proposed) → [details](/.rulebook/decisions/006-tml-first-runtime-migrating-away-from-c-c.md)
- **ADR-007: Zig CC as Preferred Compiler Toolchain** (proposed) → [details](/.rulebook/decisions/007-zig-cc-as-preferred-compiler-toolchain.md)
- **ADR-008: LL(1) Grammar with Single-Token Lookahead** (proposed) → [details](/.rulebook/decisions/008-ll-1-grammar-with-single-token-lookahead.md)
...
- **30-Second Codegen Timeout Watchdog** → [spec](/.rulebook/knowledge/patterns/30-second-codegen-timeout-watchdog.md)
- **Content-Addressable Object Cache** → [spec](/.rulebook/knowledge/patterns/content-addressable-object-cache.md)
- **Error Propagation with ! Operator (not ? like Rust)** → [spec](/.rulebook/knowledge/patterns/error-propagation-with-operator-not-like-rust.md)
- **Fat Pointer Thin/Capture Branch for Closure Calls** → [spec](/.rulebook/knowledge/patterns/fat-pointer-thin-capture-branch-for-closure-calls.md)
- **Hoisted Alloca at Function Entry** → [spec](/.rulebook/knowledge/patterns/hoisted-alloca-at-function-entry.md)
- **Lazy Monomorphization Queues for Generics** → [spec](/.rulebook/knowledge/patterns/lazy-monomorphization-queues-for-generics.md)
- **On-Demand Runtime Declaration Emission** → [spec](/.rulebook/knowledge/patterns/on-demand-runtime-declaration-emission.md)
- **Quality over speed — complete implementation approach** → [spec](/.rulebook/knowledge/patterns/quality-over-speed-complete-implementation-approach.md)
- **SEH-to-C++ Exception Translation for Crash Isolation** → [spec](/.rulebook/knowledge/patterns/seh-to-c-exception-translation-for-crash-isolation.md)
- **Stride-Based Generic Element Storage in Collections** → [spec](/.rulebook/knowledge/patterns/stride-based-generic-element-storage-in-collections.md)
- **Swiss Table-style Control Bytes in HashMap** → [spec](/.rulebook/knowledge/patterns/swiss-table-style-control-bytes-in-hashmap.md)
- **User-Defined Decorators with Quote/Splice** → [spec](/.rulebook/knowledge/patterns/user-defined-decorators-with-quote-splice.md)
- **VTable Deduplication via vtable_content_to_name_** → [spec](/.rulebook/knowledge/patterns/vtable-deduplication-via-vtable-content-to-name.md)

### Anti-Patterns to Avoid

- **Closures Always Heap-Allocate Capture Environment** → [details](/.rulebook/knowledge/anti-patterns/closures-always-heap-allocate-capture-environment.md)
- **Error Recovery Returns "0" Sentinel in Codegen** → [details](/.rulebook/knowledge/anti-patterns/error-recovery-returns-0-sentinel-in-codegen.md)
- **LLVMIRGen God Class with 50+ Mutable State Fields** → [details](/.rulebook/knowledge/anti-patterns/llvmirgen-god-class-with-50-mutable-state-fields.md)
- **parse_mangled_type_string Duplication (9+ copies)** → [details](/.rulebook/knowledge/anti-patterns/parse-mangled-type-string-duplication-9-copies.md)
- **Path Selection Forces Legacy for Any File with Imports** → [details](/.rulebook/knowledge/anti-patterns/path-selection-forces-legacy-for-any-file-with-imports.md)
- **Shortcuts, stubs, placeholders, and simplified logic** → [details](/.rulebook/knowledge/anti-patterns/shortcuts-stubs-placeholders-and-simplified-logic.md)
- **String-Based Type Encoding in Legacy Codegen** → [details](/.rulebook/knowledge/anti-patterns/string-based-type-encoding-in-legacy-codegen.md)
...
### Lua Development Rules

For comprehensive Lua-specific guidelines, see `/.rulebook/specs/LUA.md`
...
| Languages | Lua | Execute these commands after EVERY implementation (see AGENT_AUTOMATION module for full workflow). |
| Languages | Cpp | **CRITICAL**: Execute these commands after EVERY implementation (see AGENT_AUTOMATION module for full workflow). |
| Languages | Lua | **CRITICAL**: Execute these commands after EVERY implementation (see AGENT_AUTOMATION module for full workflow). |
...
<!-- RULEBOOK:SKILL:languages/lua:START -->
<!-- LUA:START -->
# Lua Project Rules
...
<!-- RULEBOOK:SKILL:languages/lua:END -->
...
<!-- OVERRIDE:START -->
# TML Project Overrides

**These directives take precedence over all generated AGENTS.md rules.**

## TML MCP Server — Mandatory Tool Usage

The TML compiler exposes a dedicated MCP server (`tml_mcp.exe`) with 20+ tools. **Every tool call is logged for LLM debugging research.**

### RULE: Always Use MCP Tools Instead of Bash

| Operation | Use MCP Tool | NEVER Use Bash |
|-----------|-------------|----------------|
| Run tests | `mcp__tml__test` | ~~`tml.exe test`~~ |
| Compile | `mcp__tml__compile` | ~~`tml.exe build`~~ |
| Run program | `mcp__tml__run` | ~~`tml.exe run`~~ |
| Type check | `mcp__tml__check` | ~~`tml.exe check`~~ |
| Emit LLVM IR | `mcp__tml__emit-ir` | ~~`tml.exe build --emit-ir`~~ |
| Emit MIR | `mcp__tml__emit-mir` | ~~`tml.exe build --emit-mir`~~ |
| Format code | `mcp__tml__format` | ~~`tml.exe format`~~ |
| Search docs | `mcp__tml__docs_search` | ~~`grep docs/`~~ |
| Build compiler | `mcp__tml__project_build` or `scripts\build.bat` | ~~`cmake --build`~~ |

The ONLY exception: `scripts\build.bat` for building the compiler itself from C++.

### Complete Tool Catalog

**Compilation & Execution:**
- `mcp__tml__compile` — Compile .tml (`file`, `optimize`, `release`)
- `mcp__tml__build` — Full build (`file`, `optimize`, `release`, `crate_type`)
- `mcp__tml__run` — Build and execute (`file`, `args`, `release`)
- `mcp__tml__check` — Type check only (`file`) — **fastest error detection**

**Testing:**
- `mcp__tml__test` — Run tests with:
  - `suite="core/str"` — module filter
  - `path="lib/core/tests/str/basic.test.tml"` — single file
  - `filter="split"` — name match
  - `structured=true` — JSON results `{total, passed, failed, failures[]}`
  - `debug_layers=true` — **emit HIR + MIR + LLVM IR on failure with diagnosis hints**
  - `coverage=true`, `no_cache=true`, `fail_fast=true`, `profile=true`

**Diagnostics (IR-Level):**
- `mcp__tml__emit-ir` — LLVM IR (`file`, `function`, `offset`, `limit`, `optimize`)
- `mcp__tml__emit-mir` — MIR (`file`)
- `mcp__tml__explain` — Error code explanation (`code` e.g. "T001")

**Documentation:**
- `mcp__tml__docs_search` — Search (`query`, `kind`, `module`, `mode`, `limit`)
- `mcp__tml__docs_get` — Full item docs (`id` e.g. "core::str::split")
- `mcp__tml__docs_list` — Module contents (`module` e.g. "std::sync")
- `mcp__tml__docs_resolve` — Resolve name to path (`name` e.g. "HashMap")

**Code Quality:**
- `mcp__tml__format` — Format (`file`, `check`)
- `mcp__tml__lint` — Lint (`file`, `fix`)
- `mcp__tml__cache_invalidate` — Clear stale cache (`files`)

**Project:**
- `mcp__tml__project_build` — Build compiler (`mode`, `target`, `clean`)
...
- `mcp__tml__project_affected-tests` — Tests for git changes (`base`, `run`)
- `mcp__tml__project_artifacts` — Build outputs (`config`, `kind`)
- `mcp__tml__project_slow-tests` — Slow test analysis (`sort`, `limit`)

### Debugging Decision Tree

```
Test failing?
├─ Compilation error → mcp__tml__check or mcp__tml__emit-ir
...
│  ├─ LLVM IR wrong → MirCodegen bug (emit_call_inst, types)
│  └─ All IR correct → C runtime or TML library logic bug
├─ Crash → mcp__tml__test with debug_layers=true (check for ABI mismatch)
└─ Unknown → mcp__tml__explain with error code
```

### Key Directives

1. **ALWAYS search docs** before writing TML code: `mcp__tml__docs_search`
2. **Use `structured=true`** on test calls for machine-readable output
3. **Use `debug_layers=true`** when debugging codegen/runtime failures
4. **Use `check`** before `emit-ir` — faster, catches type errors without codegen
5. **Never run `tml.exe` via Bash** when the equivalent MCP tool exists
6. **Every call is logged** to `mcp-call-log.jsonl` for research

### Environment Variables

| Variable | Effect |
|----------|--------|
| `TML_DEBUG_LAYERS=1` | Makes `--debug-layers` default in MCP test tool |
| `TML_MCP_LOG_DIR=/path` | Override log file directory |
...
> **Project context**: Primary language is **lua**. Agents are pre-configured with this context.
