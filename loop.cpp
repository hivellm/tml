...
auto LLVMIRGen::gen_for_iterator(const parser::ForExpr& for_expr,
    ...
    if (maybe_llvm_type == "void" || maybe_llvm_type == "ptr") {
        ...
        emit_line("  " + next_result + " = call " + maybe_llvm_type + " @" + next_fn +
                  "(ptr " + iter_alloca + ")");

        // Extract tag (field 0, i32)
        std::string tag_val = fresh_reg();
        emit_line("  " + tag_val + " = extractvalue " + maybe_llvm_type + " " + next_result + ", 0");
        ...
        emit_line("  " + item_val + " = extractvalue " + maybe_llvm_type + " " + next_result + ", 1");
