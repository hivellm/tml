...
- [x] 2.1 Update or create documentation covering the implementation — `docs/analysis/tml-table-analysis/07-determinism-baseline.md` (numbers + interpretation + reproduce section), env-var contract documented in `mem.c` block comment and both script headers, patch notes v0.3.53
- [x] 2.2 Write tests covering the new behavior — 5 corpus `.test.tml` suites (checksum-validating canaries), `scripts/determinism-selftest.sh` (double-free detection, clean-run under flags, runner enforcement), `scripts/fixtures/double_free_probe.tml` (kept OUT of test discovery — a deliberately-crashing fixture under compiler/tests/ was picked up as an X003 suite; moved to scripts/fixtures/)
- [x] 2.3 Run tests and confirm they pass — core/alloc 41/41, determinism suite 5/5, selftest 3/3, gate 17/17 targets at floor (10 runs, adversarial ON)
