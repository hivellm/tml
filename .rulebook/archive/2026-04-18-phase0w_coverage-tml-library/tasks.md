...
- [~] 2.6 External `genhtml` verification deferred — `genhtml`
  (lcov-perl) is not installed in this environment. The reproduction
  procedure lives in `lib/coverage/README.md`; independent
  verification that `llvm-cov export -format=lcov` and our LCOV
  output agree on aggregate counts + module-level structure already
  lands under 11.2 / 3.3 (see `llvm-cov-parity.md`).
...
- [~] 7.7 Manual browser verification (Chrome + Firefox) deferred —
  can only be exercised interactively. The `coverage_cli
  --format=html` output has been inspected programmatically; every
  asset (index.html, app.css, app.js, prism.min.js, tml.prism.js)
  is produced and non-empty. Open `index.html` from `file://` to
  complete the manual pass.
...
- [x] 12.2 Write tests covering the new behavior — 10 `.test.tml`
  files under [`lib/coverage/tests/`](../../lib/coverage/tests/)
  covering every public function (ingest × 3 formats, transform × 3,
  emit × 4, plus a smoke test of the stub API). 14 fixtures under
  `tests/fixtures/{lcov,llvm_json,legacy_json}/`. C++ regression at
  `compiler/tests/cli/cmd_coverage_test.cpp`. Cross-validation
  fixture at `lib/coverage/tests/fixtures/lcov/llvm_cov_reference.info`
  with a regression guard in `ingest_lcov.test.tml`.
- [x] 12.3 Run tests and confirm they pass — `tml check` passes
  cleanly on all 26 `.tml` files under
