...
- [x] 7.1 Update or create documentation covering the implementation — `docs/analysis/string/README.md` updated (F-004 resolved to "phase 1k length-prefix, 26 ns/op, 13x vs Rust"; Phase 1k section documents the 24-byte header layout, magic sentinel rationale, and the 15 migrated stdlib producers)
- [x] 7.2 Write tests covering the new behavior — `benchmarks/profile_tml/string_bench.tml` exercises every path: empty-literal init (fresh alloc), literal→heap promotion, heap→heap concat (in-place + grow), `tml_str_free` on literal, `tml_str_free` on prefixed (magic-gated -24). `bench_int_to_str` / `bench_log_naive` cover the stdlib migration. `core_str_str_methods` regression suite validates `Str.len()` binding swap.
- [x] 7.3 Run tests and confirm they pass — three consecutive bench runs pass (exit 0) with stable timings 26/26/27 ns/op; `core_str_str_methods` 2/2 pass (no-cache); VERSION remains at 0.3.36
