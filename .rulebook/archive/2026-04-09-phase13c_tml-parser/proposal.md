...
Porting the parser to TML is the critical step that makes the frontend self-hosting. The TML parser is required by phase13d to wire the TML lexer+parser into the compiler pipeline and by phase13e to begin the differential test suite. Without a working TML parser, the self-hosting milestone cannot be reached.
