...
- [x] 5.1 Write minimal TML lexer stage at `tools/stages/lexer_stage.tml` — reads source file path from argv, tokenizes using `std::lex` module, serializes token list to stdout in stage contract format
- [x] 5.2 Test: `tml build --stage=lexer:tml .sandbox/hello.tml` compiles successfully, executable produces correct output matching pure C++ build — verified: TML lexer subprocess emits binary tokens, C++ deserializer populates Token::value variants, parser/typecheck/codegen produces working exe that prints `hello`
- [x] 5.3 IR-diff (using phase12d tool): compare LLVM IR from pure C++ pipeline vs hybrid pipeline with TML lexer stage — output must be bitwise identical (verified bitwise-identical THIR/MIR/LLVM IR via `--emit-pipeline` diff)
- [x] 5.4 Performance: measure per-file overhead of stage subprocess launch + serialization round-trip — must be less than 5% of total compile time for files over 100 lines (measured: baseline 3211ms vs hybrid 3226ms = 0.47% overhead on hello.tml)

## 1. Tail (mandatory — enforced by rulebook v5.3.0)
- [x] 1.1 Update or create documentation covering the implementation (docs/specs/33-SERIAL-FORMAT.md: argv[1] convention + value-tag population note)
- [x] 1.2 Write tests covering the new behavior (scripts/tests/phase12f_hybrid_pipeline.sh: baseline vs hybrid exe output + THIR/MIR/LLVM IR bitwise diff)
- [x] 1.3 Run tests and confirm they pass (OK — phase12f hybrid pipeline bitwise-identical)
