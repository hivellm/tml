...
- [x] 2.1 Update or create documentation covering the implementation (2026-07-19): zero-copy + invalidation docstrings on every new accessor/iterator (List/HashMap/BTreeMap/Deque get_ref/get_mut, ListRefIter, value_ref, as_str_ref); F-019 by-value-vs-ref coherence note on iter()/value(); `docs/patches/v0.3.81.md` + CHANGELOG row; Rust-parity analysis `docs/analysis/tml-table-analysis/09-borrow-accessors-perf.md`.
- [x] 2.2 Write tests covering the new behavior (2026-07-19): `iter_ref.test.tml` (E), `text/text_as_str_ref.test.tml` (G1), `text/text_concat_nocopy.test.tml` (G2), plus Cluster B/C accessor tests (`{list,hashmap,btree_deque}_get_ref`), `list_interior_ref` positive borrow shapes, `scripts/fixtures/borrow/` negative fixtures via `compiler/tests/cli/borrow_interior_ref.sh`, `d0_row_collision` regression, un-stubbed `list_index_mut` + `non_null_refs`.
- [x] 2.3 Run tests and confirm they pass (2026-07-19 final): std/collections 102/102, core/str 33/33, core/hash 14/14, std/json 23/23, std/text 0 fail, compiler/borrow green + 2/2 B009 negative rejections, core/alloc 44/45 (pre-existing floating flaky, victim rotates, green standalone). Determinism gate adversarial ×100: 28/28 at/above floors (gate exit 0).
