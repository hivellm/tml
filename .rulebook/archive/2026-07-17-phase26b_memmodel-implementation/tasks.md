...
- [x] 2.1 Update or create documentation covering the implementation — `docs/adr/ADR-009-memory-model-soundness.md` gained an **Implementation status** table (plan item → landed-as → version, v0.3.55–63, incl. the move_value-dead-code discovery that re-sequenced 2.4 into phase26f); `docs/analysis/tml-table-analysis/08-memory-copy-audit.md` gained a **Closure status** table (F-013..F-023 each with status + closing version/canary, plus the audit-missed `let b = a` double-drop found by the facts). Specs already in-task: `specs/step2-scoping/{spec,wiring-plan}.md`. Commit `8d8ff260`.
- [x] 2.2 Write tests covering the new behavior — executable canaries in the determinism corpus (regression-guarded forever, adversarial allocator ON): `tml_refcount_bleed_userpath` (Step 1), `f016_list_iter`/`f016_retain`×2/`f016_arc_makemut`/`f022_destroy_releases` (Step 3), `tml_let_move_double_drop` + `tml_container_field_methodarg` (2.4/3.4 via phase26f). Framework tests: `compiler/tests/determinism/*.test.tml` (@test framework). Join-proof harness behind `TML_DROP_FACTS_DEBUG=1`.
- [x] 2.3 Run tests and confirm they pass — gates green at each step's landing (recorded per-item above), final state at v0.3.63: determinism adversarial **24/24** at/above floor; core/alloc 44/44, core/str green, std/json green, core/sync green, collections at baseline; all failures encountered during verification proven pre-existing at HEAD (backup/checkout/restore round-trips) and catalogued in `scripts/known-failures.txt` (shrink-only).
