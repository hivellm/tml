...
**Status**: Archived — 86% complete (214 → ~29). Remaining work split into dedicated tasks:
- `phase0h_closure-type-preservation` — 18 RC7 deeper tests (ClosureType lost in struct instantiation)
- ~~`phase0i_behavior-fqn-keying`~~ — **FIXED** (2026-04-08): `io_write_basic` family fixed by FQN keying for behavior maps; `core::io::Write` / `core::fmt::Write` collision resolved.
- Remaining ~10 failures are either false positives (MODULE_NOT_FOUND needs `use` fix, NEON needs platform gating, TIMEOUT is heavy tests) or small isolated bugs to be filed individually.
**Depends on**: None
