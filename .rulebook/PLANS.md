...
<!-- PLANS:HISTORY:START -->
### 2026-07-19
phase26g_collection-mutator-mut-this COMPLETED + ARCHIVED (v0.3.82, commits f055e9b1 + 7e55d160). User decision #13: Rust-style mut this. Delivered: List/HashMap/Buffer mutators flipped (36 signatures; BTreeMap/Deque/HashSet-family already mut this). KEY MEASUREMENT: no call-site cascade — owned `let` bindings may call `mut this` methods in this compiler (binding-level `var` enforcement is NOT in the type checker; recorded as future decision in v0.3.82 notes). Borrow-checker completion: recv_is_mut_this was computed-and-discarded in the phase26e wiring — now a mut this call takes a transient unique borrow conflicting with live interior refs (B009); phantom-borrow false positive fixed (inline-consumed refs); Maybe[ref V] propagates through when Just(r) arms AND let-else (LetElseStmt was entirely unvisited by the borrow checker before — now checked). get_ref-then-mutate is a B009 compile error on ALL FOUR collections. borrow_interior_ref.sh 6/6, zero new borrow errors in sweep, determinism 28/28. known-failures.txt: added std/text/text_search_transform K001 (proven pre-existing phase27a Class 2, path-sensitive — emit-ir valid, test-compile codegen_unit trips), removed 3 stale std/collections lines (fixed at d39333dd). ERA-0 arc this session: phase26c (v0.3.80) + phase26e (v0.3.81) + phase26g (v0.3.82) all complete. NEXT pending: phase27a (formally open; Class 2 partially landed at HEAD; Classes 1/3/4/5/6 remain per its spec), phase27b/c, phase44a (dispatcher false-pass), phase28a UzDB soak.

### 2026-07-19
phase26e_collection-borrow-accessors COMPLETED + ARCHIVED (v0.3.81; commits 02781659, 5b7b8270, e3e4fe81, 84e3507e, 6735e62f, ced02379). F-021 CLOSED — TML collections have a real zero-cost borrow API with Rust-parity IR evidence (docs/analysis/tml-table-analysis/09-borrow-accessors-perf.md). Delivered: (A) three dead intrinsics implemented (list_get_mut, ptr_as_ref, ptr_as_mut — all were the silent-i32-default hole), IndexMut end-to-end (list[i]=v; root cause was check_index typing container indexing as unit), C027 fixed for get_ref().field chains; (B/C) get_ref/get_mut on List (ref T), HashMap/BTreeMap (Maybe[ref V] — verified working), Deque; refcount probes prove no bump on borrow reads; (D) interior-ref borrow checking: ref-returning method results bind to the receiver borrow, conflicting interior refs = B009, ZERO false positives on the corpus; D0 fixed bare-name struct-layout shadowing (local Row inheriting sqlite::Row layout); (E/F/G) List::iter_ref + HashMapIter::value_ref (F-019: by-value stays documented clone-read, ref is opt-in), hot sites migrated (contains/sort/find_index/duplicate — moves stay moves; TRAP: ref operands do NOT auto-deref in binary ops, use *get_ref), Text::as_str_ref zero-copy for heap Text + concat double-copy killed. Final gates: std/collections 102/102, str/hash/json green, borrow suite + 2/2 B009 negatives, core/alloc at baseline (floating flaky), determinism adversarial ×100 28/28. RESIDUAL → phase26g_collection-mutator-mut-this (USER DECISION: mut this migration vs @invalidates_refs attribute — get_ref-then-push detection blocked on it; blast radius measured 20+ std tests). Learnings saved (ref API facts + traps). NEXT pending tasks: phase27a (partially landed at HEAD), phase27b/c, phase26g (needs user), phase44a (dispatcher false-pass).
