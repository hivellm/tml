...
- [x] 1.1 Documentation: all 7 parser files have module-level `//!` doc comments; `common.tml` explains the ParsedX pattern and positional threading convention
- [x] 1.2 Tests: 4 targeted sub-parser tests passing; `parser_basic.test.tml` written for future use when codegen timeout limit is raised
- [x] 1.3 Tests confirmed passing: `parse_type_path`, `parse_type_path_full`, `parse_named_type`, `parse_type` — all return 0 (success) against valid TML source strings
