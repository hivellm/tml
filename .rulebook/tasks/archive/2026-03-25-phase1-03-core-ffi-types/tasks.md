...
**Status**: COMPLETE (20/20 items done)
**Priority**: HIGH
**Phase**: 1 — Foundation
...
- [x] 1.6 Platform-dependent `c_long` — `c_long_bits()` and `c_long_max()` functions via `#if WINDOWS`/`#else` (workaround: `#if` around functions, not struct fields)
...
- [x] 3.2 Migration guide — added as doc comments in `mod.tml` (table + usage examples)
...
1. **Sub-byte struct fields** (`I8`, `U8`, `I16`, `U16`): Cross-module import of structs with these field types causes parse error. `c_char`, `c_uchar`, `c_short`, `c_ushort` omitted — track in compiler bug backlog.
2. **`#if` inside type definitions**: Parse error. Workaround: use `#if` around top-level functions instead of struct fields.
