...
- [ ] 1.6 HTTP/2 Buffer family — DEFERRED to next increment: the real web is ~35 edits across 6 files (12 `h2_encode_frame_raw` callers incl. `frame.payload` field-access refs, `process_frame` external/test callers). SCOPE CORRECTION vs the mapper: `H2Frame::new`/`data` STORE the payload into the struct — consuming constructors, correctly by-value, must NOT be migrated. `controller.tml:255 register_all` rides with this increment
