...
### Step 3 — Sound container / smart-pointer read-out (ACTIVE — closes the real bugs, no move semantics needed)
- [ ] 3.1 Read-outs → balanced clone: migrate the by-value yields to `ptr_read_clone[T]` (bump paired with the returned value's drop; bitwise for primitives) — `ListIter::next` (`behaviors.tml:349`), `HashSetIter::next/current/next_item` (`behaviors.tml:533`, `class_collections.tml:315,334`), `HashMapIter::value` (`hashmap.tml:636`), `List::retain` local (`list.tml:282`). Also `Sync::get`/`Heap::get` for consistency with `List/HashMap::get`
- [ ] 3.2 `Arc::make_mut` (`arc.tml:381`): CoW must deep-clone (`data.duplicate()`), not bitwise-copy, so old+new don't alias nested handles
- [ ] 3.3 F-022: `List::destroy`/`HashMap::destroy` run per-element `Drop` (iterate + drop each element) before `mem_free` of the buffer, so element handles are released
...
- [ ] 3.5 Gates: 13 F-016 sites + F-022 have new canaries (nested-handle in container, iterate, destroy) at 100/100 both modes; determinism gate + affected suites at/above baseline; K002 clean
