...
- [x] 1.3 On store to Bool struct field: emit `zext i1 to i8` then `store i8` — llvm_struct_expr.cpp (3 store paths) + instructions_misc.cpp coerce_int_type + binary.cpp field assign + instructions.cpp StoreInst handler
...
- [x] 2.3 Verify existing tests still pass — 227/227 compiler tests (Cell[Bool] generic path fixed in binary.cpp)
