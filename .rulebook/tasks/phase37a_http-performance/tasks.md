...
**Status**: BLOCKED — Phase 0 regression not investigated. 35% (8/23).
**Current**: 14K autocannon (fixed), 8K bombardier (REGRESSION from 183K — UNINVESTIGATED)
**Reference**: docs/analysis/http/00-strategic-plan.md

## Phase 0: Fix Performance Regression (CRITICAL)

⚠️ CRITICAL BLOCKER: This regression has been pending since profiler instrumentation was added. Root cause suspected: profiler::begin/end calls in hot paths (str, option, hashmap). Must be resolved before ANY other phase can proceed.
