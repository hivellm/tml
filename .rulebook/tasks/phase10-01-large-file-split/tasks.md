...
| Metric | Value |
|--------|-------|
| Files split/refactored | 13 |
| New files created | 24 |
| Total lines reduced | ~7,800 lines removed from originals |
| Largest reduction | `generic_instantiate.cpp` 1,738 → 663 (-62%) |
| Skipped (not splittable) | 7 |

## Phase 1: Codegen (highest ROI — most edited files) — 6/7
...
- [ ] 1.3 Skip — `llvm_ir_gen.hpp` (1,873 lines) is a header, needs architectural redesign (PIMPL, partial headers)
...
- [ ] 2.1 Skip — `env_module_load.cpp` (1,543 lines) is one giant function with many static helper dependencies, needs dedicated refactoring
...
- [ ] 3.1 Skip — `testing_compile.cpp` (1,824 lines) uses file-local statics with shared state, split requires linkage changes
...
- [x] 4.1 Split `os.c` (1,736 → 1,120, -35%) → `os_process.c` (647) — subprocess, signal, pipes
- [x] 4.2 Split `essential.c` (1,719 → 1,667, -3%) → `essential_cpuid.c` (74) — CPUID/CPU feature detection

## Phase 5: TML Library — 0/2

- [ ] 5.1 Skip — `str.tml` (2,017 lines) — standalone pub funcs, split would change module public API
- [ ] 5.2 Skip — `atomic.tml` (1,507 lines) — same reason as 5.1
