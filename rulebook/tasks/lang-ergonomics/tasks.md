...
- [x] 7.1.1 Analyze current type inference limitations
- [x] 7.1.2 Implement RHS-to-LHS type propagation for let
- [x] 7.1.3 Improve generic type inference from arguments
- [x] 7.1.4 Add unit tests for inferred type declarations
- [x] 7.1.5 Test cipher.tml compiles with fewer annotations

## Phase 8: Tuple Destructuring ✓

- [x] 8.1.1 Parse tuple patterns `(a, b)` in let statements
- [x] 8.1.2 Parse tuple patterns in when arms
- [x] 8.1.3 Parse tuple patterns in function parameters
- [x] 8.1.4 Implement type checking for tuple destructuring
...
- [x] 9.1.3 Validate default types match field types
...
- [x] 11.1.1 Detect last expression without semicolon in blocks (already implemented)
- [x] 11.1.2 Convert to implicit return in HIR lowering (already implemented)
- [x] 11.1.3 Handle if/when expressions as implicit returns (already implemented)
- [x] 11.1.4 Add unit tests for implicit returns
- [x] 11.1.5 Update documentation with implicit return rules (N/A - behavior matches Rust)

## Phase 12: Struct Field Iteration (Compile-Time Reflection) ✓

**Implementation:** Uses existing `field_count[T]()`, `field_name[T](i)`, `field_offset[T](i)` intrinsics with automatic loop unrolling when iterating over `for i in 0 to field_count[T]()`.

- [x] 12.1.1 Design syntax for field iteration (`for i in 0 to field_count[T]()`)
- [x] 12.1.2 Existing `field_count`, `field_name`, `field_offset` intrinsics work
...
- [x] 13.1.1 Add `union` keyword to lexer
- [x] 13.1.2 Parse union declarations with field list syntax
- [x] 13.1.3 Add UnionDecl AST node similar to StructDecl
- [x] 13.1.4 Implement type checking for union types
- [x] 13.1.5 Calculate union size as max of all field sizes
- [x] 13.1.6 Calculate union alignment as max of all field alignments
- [x] 13.1.7 Generate LLVM IR union type (single allocation, field access via bitcast)
- [x] 13.1.8 Implement field read codegen (load from union pointer)
- [x] 13.1.9 Implement field write codegen (store to union pointer)
- [x] 13.1.10 Union field access works without lowlevel block (safe by design)
- [x] 13.1.11 Union size computed automatically from max field size
- [x] 13.1.12 Add unit tests for basic union declaration and access
- [x] 13.1.13 Add unit tests for union with different sized fields
