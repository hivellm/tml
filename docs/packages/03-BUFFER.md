...
```tml
use std::buffer
use std::buffer.{Buffer, ByteReader, ByteWriter}
```
...
extend Buffer {
    ...
    /// Create from bytes
    pub func from_bytes(bytes: ref [U8]) -> This {
    ...
    /// Create from List
    pub func from_list(list: List[U8]) -> This {
    ...
    /// Current position
    pub func position(this) -> U64 {
        return this.position
    }

    /// Set position
    pub func set_position(this, pos: U64) {
    ...
    /// Extend with bytes
    pub func extend(this, bytes: ref [U8]) {
        this.data.extend_from_slice(bytes)
    }

    /// Split at position
    pub func split_off(this, at: U64) -> Buffer {
...
```tml
pub type ByteReader[R: Read] {
    inner: R,
    buf: [U8; 8],
}

extend ByteReader[R: Read] {
    pub func new(inner: R) -> This {
        return This { inner: inner, buf: [0; 8] }
    }

    /// Read exact number of bytes
    pub func read_exact(this, buf: mut ref [U8]) -> Outcome[Unit, IoError] {
        this.inner.read_exact(buf)
    }

    /// Read single byte
    pub func read_u8(this) -> Outcome[U8, IoError] {
        this.inner.read_exact(mut ref this.buf[0 to 1])!
        return Ok(this.buf[0])
    }

    /// Read signed byte
    pub func read_i8(this) -> Outcome[I8, IoError] {
        return Ok(this.read_u8()! as I8)
    }

    // Little-endian reads
    pub func read_u16_le(this) -> Outcome[U16, IoError] {
        this.inner.read_exact(mut ref this.buf[0 to 2])!
        return Ok(U16.from_le_bytes([this.buf[0], this.buf[1]]))
    }

    pub func read_u32_le(this) -> Outcome[U32, IoError] {
        this.inner.read_exact(mut ref this.buf[0 to 4])!
        return Ok(U32.from_le_bytes([this.buf[0], this.buf[1], this.buf[2], this.buf[3]]))
    }

    pub func read_u64_le(this) -> Outcome[U64, IoError] {
        this.inner.read_exact(mut ref this.buf)!
        return Ok(U64.from_le_bytes(this.buf))
    }

    pub func read_i16_le(this) -> Outcome[I16, IoError] {
        return Ok(this.read_u16_le()! as I16)
    }

    pub func read_i32_le(this) -> Outcome[I32, IoError] {
        return Ok(this.read_u32_le()! as I32)
    }

    pub func read_i64_le(this) -> Outcome[I64, IoError] {
        return Ok(this.read_u64_le()! as I64)
    }

    pub func read_f32_le(this) -> Outcome[F32, IoError] {
        return Ok(F32.from_bits(this.read_u32_le()!))
    }

    pub func read_f64_le(this) -> Outcome[F64, IoError] {
        return Ok(F64.from_bits(this.read_u64_le()!))
    }

    // Big-endian reads
    pub func read_u16_be(this) -> Outcome[U16, IoError] {
        this.inner.read_exact(mut ref this.buf[0 to 2])!
        return Ok(U16.from_be_bytes([this.buf[0], this.buf[1]]))
    }

    pub func read_u32_be(this) -> Outcome[U32, IoError] {
        this.inner.read_exact(mut ref this.buf[0 to 4])!
        return Ok(U32.from_be_bytes([this.buf[0], this.buf[1], this.buf[2], this.buf[3]]))
    }

    pub func read_u64_be(this) -> Outcome[U64, IoError] {
        this.inner.read_exact(mut ref this.buf)!
        return Ok(U64.from_be_bytes(this.buf))
    }

    pub func read_i16_be(this) -> Outcome[I16, IoError]
    pub func read_i32_be(this) -> Outcome[I32, IoError]
    pub func read_i64_be(this) -> Outcome[I64, IoError]
    pub func read_f32_be(this) -> Outcome[F32, IoError]
    pub func read_f64_be(this) -> Outcome[F64, IoError]

    // Variable-length integers (LEB128)
    pub func read_varint(this) -> Outcome[I64, IoError] {
        var result: I64 = 0
        var shift: U32 = 0
    ...
    pub func read_uvarint(this) -> Outcome[U64, IoError] {
        var result: U64 = 0
        var shift: U32 = 0
    ...
    // Strings
    pub func read_string(this, len: U64) -> Outcome[String, IoError] {
    ...
    pub func read_cstring(this) -> Outcome[String, IoError] {
        var bytes = List.new()
        loop {
            let b = this.read_u8()!
            if b == 0 {
    ...
    pub func read_length_prefixed_string(this) -> Outcome[String, IoError] {
...
extend ByteWriter[W: Write] {
    ...
    /// Write single byte
    pub func write_u8(this, v: U8) -> Outcome[Unit, IoError] {
        this.buf[0] = v
        this.inner.write_all(&this.buf[0 to 1])
    }

    /// Write signed byte
    pub func write_i8(this, v: I8) -> Outcome[Unit, IoError] {
        this.write_u8(v as U8)
    }

    // Little-endian writes
    pub func write_u16_le(this, v: U16) -> Outcome[Unit, IoError] {
        let bytes = v.to_le_bytes()
        this.inner.write_all(&bytes)
    }

    pub func write_u32_le(this, v: U32) -> Outcome[Unit, IoError] {
        let bytes = v.to_le_bytes()
        this.inner.write_all(&bytes)
    }

    pub func write_u64_le(this, v: U64) -> Outcome[Unit, IoError] {
        let bytes = v.to_le_bytes()
        this.inner.write_all(&bytes)
    }

    pub func write_i16_le(this, v: I16) -> Outcome[Unit, IoError]
    pub func write_i32_le(this, v: I32) -> Outcome[Unit, IoError]
    pub func write_i64_le(this, v: I64) -> Outcome[Unit, IoError]
    pub func write_f32_le(this, v: F32) -> Outcome[Unit, IoError]
    pub func write_f64_le(this, v: F64) -> Outcome[Unit, IoError]

    // Big-endian writes
    pub func write_u16_be(this, v: U16) -> Outcome[Unit, IoError]
    pub func write_u32_be(this, v: U32) -> Outcome[Unit, IoError]
    pub func write_u64_be(this, v: U64) -> Outcome[Unit, IoError]
    pub func write_i16_be(this, v: I16) -> Outcome[Unit, IoError]
    pub func write_i32_be(this, v: I32) -> Outcome[Unit, IoError]
    pub func write_i64_be(this, v: I64) -> Outcome[Unit, IoError]
    pub func write_f32_be(this, v: F32) -> Outcome[Unit, IoError]
    pub func write_f64_be(this, v: F64) -> Outcome[Unit, IoError]

    // Variable-length integers
    pub func write_varint(this, v: I64) -> Outcome[Unit, IoError] {
        var value = v
        loop {
    ...
    pub func write_uvarint(this, v: U64) -> Outcome[Unit, IoError] {
        var value = v
        loop {
            let byte = (value & 0x7F) as U8
            value >>= 7
    ...
    // Strings
    pub func write_string(this, s: ref str) -> Outcome[Unit, IoError] {
        this.inner.write_all(s.as_bytes())
    }

    pub func write_cstring(this, s: ref str) -> Outcome[Unit, IoError] {
        this.inner.write_all(s.as_bytes())!
        this.write_u8(0)
    }

    pub func write_length_prefixed_string(this, s: ref str) -> Outcome[Unit, IoError] {
...
extend BufReader[R: Read] {
    ...
    pub func with_capacity(cap: U64, inner: R) -> This {
    ...
    pub func read_until(this, delim: U8, buf: mut ref List[U8]) -> Outcome[U64, IoError] {
        var read: U64 = 0
        loop {
    ...
    /// Read line (until \n)
    pub func read_line(this, buf: mut ref String) -> Outcome[U64, IoError] {
        var bytes = List.new()
...
extend BufWriter[W: Write] {
    ...
    pub func with_capacity(cap: U64, inner: W) -> This {
    ...
    pub func into_inner(this) -> Outcome[W, IoError] {
        this.flush()!
...
extend RingBuffer[T] {
    pub func new(capacity: U64) -> This {
    ...
    pub func push_back(this, value: T) -> Bool {
        if this.is_full() {
            return false
        }
    ...
    pub func push_front(this, value: T) -> Bool {
        if this.is_full() {
            return false
        }
...
use std::fs.File
use std::buffer.{BufReader, BufWriter}
