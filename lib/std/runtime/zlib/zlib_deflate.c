...
// Exported as buffer_destroy for TML lowlevel calls
void buffer_destroy(void* handle) {
    tml_buffer_destroy((TmlBuffer*)handle);
}
...
GzipHeaderInfo* gzip_read_header(TmlBuffer* data) {
    ...
    if (flags & 0x08) {  // FNAME
        ...
        while (pos < data->len && data->data[pos] != 0) pos++;
        if (pos < data->len) {
...
uint32_t crc32_combine(uint32_t crc1, uint32_t crc2, int64_t len2) {
    return (uint32_t)crc32_combine_((uLong)crc1, (uLong)crc2, (z_off_t)len2);
}

uint32_t adler32_compute(const char* data) {
    if (!data) return 1;  // Adler32 of empty is 1
    size_t len = strlen(data);
...
uint32_t adler32_update(uint32_t adl, const char* data) {
    if (!data) return adl;
    size_t len = strlen(data);
    return (uint32_t)adler32((uLong)adl, (const Bytef*)data, (uInt)len);
}

uint32_t adler32_update_buffer(uint32_t adl, TmlBuffer* data) {
    if (!data) return adl;
...
uint32_t adler32_combine(uint32_t adler1, uint32_t adler2, int64_t len2) {
    return (uint32_t)adler32_combine_((uLong)adler1, (uLong)adler2, (z_off_t)len2);
}
...
TmlBuffer* deflate_stream_write(DeflateStream* stream, const char* data, int32_t flush) {
    if (!stream || !stream->initialized || !data) return NULL;
