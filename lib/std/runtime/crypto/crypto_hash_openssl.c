...
static const EVP_MD* get_evp_md(const char* algorithm) {
    ...
    if (strcasecmp(algorithm, "sha256") == 0)
        return EVP_sha256();
    if (strcasecmp(algorithm, "sha384") == 0)
        return EVP_sha384();
    if (strcasecmp(algorithm, "sha512") == 0)
        return EVP_sha512();
    if (strcasecmp(algorithm, "sha512-256") == 0)
        return EVP_sha512_256();

#if OPENSSL_VERSION_NUMBER >= 0x10101000L
    if (strcasecmp(algorithm, "sha3-256") == 0)
        return EVP_sha3_256();
    if (strcasecmp(algorithm, "sha3-384") == 0)
        return EVP_sha3_384();
    if (strcasecmp(algorithm, "sha3-512") == 0)
        return EVP_sha3_512();
    if (strcasecmp(algorithm, "blake2b512") == 0)
        return EVP_blake2b512();
    if (strcasecmp(algorithm, "blake2s256") == 0)
...
static TmlBuffer* hash_oneshot(const char* algorithm, const uint8_t* data, size_t len) {
    ...
    int success = EVP_DigestInit_ex(ctx, md, NULL) == 1 && EVP_DigestUpdate(ctx, data, len) == 1 &&
...
TmlBuffer* crypto_sha512_256_bytes(TmlBuffer* data) {
    if (!data)
        return hash_oneshot("sha512-256", NULL, 0);
...
TmlBuffer* crypto_sha3_256_bytes(TmlBuffer* data) {
    if (!data)
        return hash_oneshot("sha3-256", NULL, 0);
...
TmlBuffer* crypto_sha3_384_bytes(TmlBuffer* data) {
    if (!data)
        return hash_oneshot("sha3-384", NULL, 0);
...
TmlBuffer* crypto_sha3_512_bytes(TmlBuffer* data) {
    if (!data)
        return hash_oneshot("sha3-512", NULL, 0);
...
TmlBuffer* crypto_blake2b512_bytes(TmlBuffer* data) {
    if (!data)
        return hash_oneshot("blake2b512", NULL, 0);
...
TmlBuffer* crypto_blake2s256_bytes(TmlBuffer* data) {
    if (!data)
        return hash_oneshot("blake2s256", NULL, 0);
