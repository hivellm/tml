...
static LPCWSTR get_pbkdf2_algorithm(const char* digest) {
    if (_stricmp(digest, "sha1") == 0)
        return BCRYPT_SHA1_ALGORITHM;
    if (_stricmp(digest, "sha256") == 0)
        return BCRYPT_SHA256_ALGORITHM;
    if (_stricmp(digest, "sha384") == 0)
        return BCRYPT_SHA384_ALGORITHM;
    if (_stricmp(digest, "sha512") == 0)
...
TmlBuffer* kdf_pbkdf2(const uint8_t* password, size_t password_len, const uint8_t* salt,
    ...
    NTSTATUS status =
        BCryptOpenAlgorithmProvider(&alg_handle, alg_id, NULL, BCRYPT_ALG_HANDLE_HMAC_FLAG);
    ...
    status = BCryptDeriveKeyPBKDF2(alg_handle, (PUCHAR)password, (ULONG)password_len, (PUCHAR)salt,
                                   (ULONG)salt_len, (ULONGLONG)iterations, result->data,
                                   (ULONG)key_len, 0);
...
TmlBuffer* crypto_pbkdf2(const char* password, TmlBuffer* salt, int64_t iterations, int64_t keylen,
                         const char* digest) {
    return kdf_pbkdf2((const uint8_t*)password, password ? strlen(password) : 0,
                      salt ? salt->data : NULL, salt ? salt->len : 0, iterations, keylen, digest);
}

TmlBuffer* crypto_pbkdf2_bytes(TmlBuffer* password, TmlBuffer* salt, int64_t iterations,
                               int64_t keylen, const char* digest) {
    return kdf_pbkdf2(password ? password->data : NULL, password ? password->len : 0,
                      salt ? salt->data : NULL, salt ? salt->len : 0, iterations, keylen, digest);
}
...
TmlBuffer* crypto_scrypt(const char* password, TmlBuffer* salt, int64_t keylen, int64_t n,
    ...
    return kdf_scrypt((const uint8_t*)password, password ? strlen(password) : 0,
                      salt ? salt->data : NULL, salt ? salt->len : 0, keylen, n, r, p, maxmem);
}

TmlBuffer* crypto_scrypt_bytes(TmlBuffer* password, TmlBuffer* salt, int64_t keylen, int64_t n,
                               int64_t r, int64_t p, int64_t maxmem) {
    return kdf_scrypt(password ? password->data : NULL, password ? password->len : 0,
                      salt ? salt->data : NULL, salt ? salt->len : 0, keylen, n, r, p, maxmem);
}
...
TmlBuffer* crypto_hkdf(const char* digest, TmlBuffer* ikm, TmlBuffer* salt, const char* info,
                       int64_t keylen) {
    return kdf_hkdf(digest, ikm ? ikm->data : NULL, ikm ? ikm->len : 0, salt ? salt->data : NULL,
                    salt ? salt->len : 0, (const uint8_t*)info, info ? strlen(info) : 0, keylen);
}

TmlBuffer* crypto_hkdf_bytes(const char* digest, TmlBuffer* ikm, TmlBuffer* salt, TmlBuffer* info,
                             int64_t keylen) {
    return kdf_hkdf(digest, ikm ? ikm->data : NULL, ikm ? ikm->len : 0, salt ? salt->data : NULL,
                    salt ? salt->len : 0, info ? info->data : NULL, info ? info->len : 0, keylen);
}
...
TmlBuffer* crypto_hkdf_extract(const char* digest, TmlBuffer* ikm, TmlBuffer* salt) {
    return kdf_hkdf_extract(digest, ikm ? ikm->data : NULL, ikm ? ikm->len : 0,
...
TmlBuffer* crypto_hkdf_expand(const char* digest, TmlBuffer* prk, TmlBuffer* info, int64_t keylen) {
    return kdf_hkdf_expand(digest, prk ? prk->data : NULL, prk ? prk->len : 0,
                           info ? info->data : NULL, info ? info->len : 0, keylen);
}
...
TmlBuffer* crypto_argon2(const char* variant, const char* password, TmlBuffer* salt, int64_t keylen,
    ...
    return kdf_argon2(variant, (const uint8_t*)password, password ? strlen(password) : 0,
                      salt ? salt->data : NULL, salt ? salt->len : 0, keylen, t, m, p);
}
...
TmlBuffer* crypto_argon2_bytes(const char* variant, TmlBuffer* password, TmlBuffer* salt,
    ...
    return kdf_argon2(variant, password ? password->data : NULL, password ? password->len : 0,
                      salt ? salt->data : NULL, salt ? salt->len : 0, keylen, t, m, p);
}
